...
<html>
    <head>
        ...
        <style>
            ...
            .dash-table-container .dash-cell:nth-child(even) {
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div([
            html.Div([
                html.H3("Filtered Data", className="data-table-title"),
                html.P("Raw ticket data based on current filter selection", 
                       className="data-table-subtitle")
            ...
            html.Div([
                dash_table.DataTable(
                    id="data-table",
                    columns=[
                        {"name": "Ticket ID", "id": "TicketID", "type": "numeric"},
                        {"name": "Date Opened", "id": "DateOpened", "type": "datetime"},
                        {"name": "Date Closed", "id": "DateClosed", "type": "datetime"},
                        {"name": "Product", "id": "Product", "type": "text"},
                        {"name": "Issue", "id": "Issue", "type": "text"},
                        {"name": "Priority", "id": "Priority", "type": "text"},
                        {"name": "Status", "id": "Status", "type": "text"},
                        {"name": "Assigned To", "id": "AssignedTo", "type": "text"},
                        {"name": "Resolution Time (h)", "id": "ResolutionTimeHours", "type": "numeric", "format": {"specifier": ".1f"}},
                        {"name": "Resolution", "id": "Resolution", "type": "text"}
                    ],
                    data=[],
...
def update_dashboard(start_date, end_date, selected_products, selected_priorities, 
    ...
    table_data["DateOpened"] = table_data["DateOpened"].dt.strftime("%Y-%m-%d %H:%M")
    table_data["DateClosed"] = table_data["DateClosed"].dt.strftime("%Y-%m-%d %H:%M")
    # Replace NaT with empty string for display
    table_data["DateClosed"] = table_data["DateClosed"].replace("NaT", "")
    # Round resolution time
    table_data["ResolutionTimeHours"] = table_data["ResolutionTimeHours"].round(1)
    
    # Select columns for display and convert to records
    display_columns = ["TicketID", "DateOpened", "DateClosed", "Product", "Issue", 
                      "Priority", "Status", "AssignedTo", "ResolutionTimeHours", 
                      "Resolution"]
    table_data = table_data[display_columns].to_dict('records')
